...
surface_variables_short = ["U10m", "V10m", "T2m", "SP"]
...
def get_surface_variable_indices():
    """Mapping from surface variable name to (var, lev) index in ERA5 dataset."""
    return {var: (i, 0) for i, var in enumerate(surface_variables_short)}


def get_level_variable_indices(pressure_levels=pressure_levels, level_variables=level_variables):
    """Mapping from level variable name to (var, lev) index in ERA5 dataset."""
    out = {}
    for var_idx, var in enumerate(level_variables):

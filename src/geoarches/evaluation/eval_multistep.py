"""Script to compute metrics over 1 year.

Example commandline:
    python -m geoarches.evaluation.eval_multistep --pred_path data/ifs_ens --output_dir evalstore/ens/ \
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Output directory to save the evaluation metrics. Required",
    )
    parser.add_argument(
        "--pred_path",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Metrics from metrics_registry.py to compute. By default asks to compute all.",
    )
    parser.add_argument(
        "--year",
        default="2020",
        type=str,
        help="Year to compute metrics on.",
    )
    parser.add_argument(
        "--eval_clim",
        action="store_true",
        help="whether to evaluate climatology. pred_path holds climatology.",
    )
    ...
    if not args.eval_clim:
        ds_pred = era5.Era5Dataset(
            path=args.pred_path,
            filename_filter=(lambda x: args.year in x),
            variables=variables,
    ...
    for metric_name, metric in metrics.items():
        ...
        scores_dict = {k: (v.cpu() if hasattr(v, "cpu") else v) for k, v in scores_dict.items()}
        if Path(args.pred_path).is_file():
            output_filename = f"{Path(args.pred_path).stem}-{metric_name}"
        else:
            output_filename = f"test{args.year}-multistep={args.multistep}-{metric_name}"

        # Write xr dataset.
        extra_dimensions = ["prediction_timedelta"]
        if "brier" in metric_name:
            extra_dimensions = ["quantile", "prediction_timedelta"]
        ds = convert_metric_dict_to_xarray(scores_dict, extra_dimensions)
        ds.to_netcdf(Path(output_dir).joinpath(f"{output_filename}.nc"))

        # Write labeled dict.
        scores_dict["groundtruth_path"] = args.groundtruth_path
        scores_dict["predictions_path"] = args.pred_path
        torch.save(scores_dict, Path(output_dir).joinpath(f"{output_filename}.pt"))

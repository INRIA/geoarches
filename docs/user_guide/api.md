...
| `exp_dir`             | `'modelstore/${name}'` | Directory for saving/loading checkpoints and configs. We **strongly** advise to **not change** this argument and instead change `name` for each new run.                                                                                                         |
| `resume`              | `False`                | Set to `True` to continue training an existing run from `exp_dir`. The experiment and checkpoint must already exist.                                                                                                                                              |
| `load_ckpt`           | _Optional_             | Experiment directory or `.ckpt` file used to initialize a new training run from existing weights. This does not resume optimizer state, scheduler state, or global step. The new `name` must not already exist.                                                   |
| `ckpt_filename_match` | _Optional_             | If specified, loads the checkpoint file whose name contains this substring. If multiple matches are found, the latest checkpoint will be loaded. Applies to `resume`, `mode=test`, and `load_ckpt` experiment directories.                                        |
| `save_step_frequency` | 50000                  | How often to save checkpoints (in steps).                                                                                                                                                                                                                                                                                        |

Training modes are intentionally strict:

- New training: use a new `name` and leave `load_ckpt` unset. `resume` should be `False` (default value).
- Continue training: use an existing `name` and set `++resume=True`.
- Fine-tuning: use a new `name` and set `++load_ckpt=<experiment-dir-or-ckpt>`. `resume` should be `False` (default value).

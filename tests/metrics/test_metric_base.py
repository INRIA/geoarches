...
class TestTensorDictMetricBase:
    def test_calls_update(self):
        ...
        mock_level_metric.update.assert_called_once_with(torch.tensor([1]), torch.tensor([3]))
        mock_surface_metric.update.assert_called_once_with(torch.tensor([2]), torch.tensor([4]))
    ...
    def test_aggregate_computed_labeled_dicts(self):
        ...
        mock_surface_metric.compute.return_value = {
            "rmse_var3": torch.tensor(5.0),
            "rmse_var4": torch.tensor(6.0),
            "mae_var3": torch.tensor(7.0),
            "mae_var4": torch.tensor(8.0),
        }
        metric = TensorDictMetricBase(level=mock_level_metric, surface=mock_surface_metric)

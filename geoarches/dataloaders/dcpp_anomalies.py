...
class DCPPAnomaliesForecast(XarrayDataset):
    """
    ...
    def __init__(
        ...
        filename_filters = dict(
            ...
            val=lambda x: any(
                substring in x
                for substring in [f"{str(x)}_" for x in [2010, 2011, 2012, 2013, 2014, 2015, 2016]]
            ),
            test=lambda x: any(
        ...
        clim_removed_norm_stats = torch.load(
            geoarches_stats_path / "climatology_stats_net_flux.pt"
        )
        climatology_residual_stats = torch.load(
            geoarches_stats_path / "climatology_residuals_stats.pt"
        )
        climatology_residual_stats["surface_std"] = torch.where(
            climatology_residual_stats["surface_std"] == 0,
            torch.nan,
            climatology_residual_stats["surface_std"],
        )
        climatology_residual_stats["level_std"] = torch.where(
            climatology_residual_stats["level_std"] == 0,
            torch.nan,
            climatology_residual_stats["level_std"],
        )
        spatial_norm_stats["surface_mean"] = torch.stack(
            [spatial_norm_stats["surface_mean"][i] for i in surface_variable_indices]
        )
        spatial_norm_stats["level_mean"] = torch.stack(
            [spatial_norm_stats["level_mean"][i] for i in level_variable_indices]
        )
        spatial_norm_stats["surface_std"] = torch.stack(
            [spatial_norm_stats["surface_std"][i] for i in surface_variable_indices]
        )
        spatial_norm_stats["level_std"] = torch.stack(
            [spatial_norm_stats["level_std"][i] for i in level_variable_indices]
        )

        # need to do this because for some variables, nans are not consistent. when we take mean over dataset we get lcd of nan mask, which we can apply to the climatology stats
        clim_removed_norm_stats["level_mean"] = torch.where(
            spatial_norm_stats["level_mean"].isnan(),
            torch.nan,
            clim_removed_norm_stats["level_mean"],
        )
        clim_removed_norm_stats["surface_mean"] = torch.where(
            spatial_norm_stats["surface_mean"].isnan(),
            torch.nan,
            clim_removed_norm_stats["surface_mean"],
        )
        ...
        # keep this for backwards compatability
            ...
            self.climatology_mean = TensorDict(
                surface=clim_removed_norm_stats["surface_mean"],
                level=clim_removed_norm_stats["level_mean"],
            )
            self.climatology_mean.batch_size = [
                12
            ]  # this is so the tensordict can be indexed by one value for both surface/level

            self.residual_mean = TensorDict(
                surface=climatology_residual_stats["surface_mean"],
                level=climatology_residual_stats["level_mean"],
            )

            self.residual_std = TensorDict(
                surface=climatology_residual_stats["surface_std"].nanmean(
                    axis=(-1, -2), keepdim=True
                ),
                level=climatology_residual_stats["level_std"].nanmean(axis=(-1, -2), keepdim=True),
            )
    ...
    def __getitem__(self, i, normalize=True):
        ...
        out["anomaly_state"] = super().__getitem__(i)
        ...
        out["next_anomaly_state"] = super().__getitem__(i + t // self.timedelta)
        ...
        if self.load_prev:
            if self.load_prev > 1:
                ...
                out["prev_anomaly_state"] = super().__getitem__(
                    i - self.lead_time_months // self.timedelta
                )
                prev_timestamp = (
        ...
        if normalize:
            out = self.normalize(out, month=current_month, timestamp=out["timestamp"])
    ...
    def normalize(self, batch, stateless=False, month=None, timestamp=None):
        device = list(batch.values())[0].device
        means = self.data_mean.to(device)
        stds = self.data_std.to(device)
        climatology_mean = self.climatology_mean.to(device)
        ...
        if stateless:
            ...
            dict_out = {
                k: ((v - means) / stds if "state" in k and "anomaly" not in k else v)
                for k, v in batch.items()
            }
            # if you check the means file there will be an offset, this is handled in the "current_month" calculation in __getitem__
            dict_out = {
                k: (
                    ((v - climatology_mean[month]) - residual_mean) / residual_std
                    if "anomaly" in k
                    else v
                )
    ...
    def denormalize(self, batch, stateless=False, month=None):
        ...
        if stateless:
            ...
            dict_out = {
                k: ((v * stds) + means if "state" in k and "anomaly" not in k else v)
                for k, v in batch.items()
            }
            dict_out = {
                k: (
                    ((v * residual_std) + residual_mean) + climatology_mean[month]
                    if "anomaly" in k
                    else v
                )
                for k, v in dict_out.items()
